...
class WideTALearningRateAdapter(TaTask0TrainTestClassificationAccAdapter, TaClassificationAccuracyAdapter):
    ...
    def clone_sp_output_tensor_for(self, layer_id: int) -> torch.Tensor:
        ...
        if type(flock) is ExpertFlockNode or type(flock) is ConvExpertFlockNode:
            ...
            return FlockNodeAccessor.get_sp_output_tensor(flock).clone().view(flock_size, -1).type(torch.float32)
